...
| 카테고리 | 발생 시점 | 규모 | 핵심 메모 |
|---|---|---|---|
| **개발 (1회성)** | M0~M5 (6~7개월) | **약 4.4~5.7억** | 8~9 FTE × 6개월 + 부대비. PIA·보험은 별도 |
| **운영 (매월 반복)** | M5(출시) 이후 | **약 2,000~3,300만/월** | 인프라+SaaS 295만 + 운영 인력 1,500~2,300만 + 보험 분할 100~300만 |
...
### 3.1 인력 비용 (Phase 1, 6개월 기준)

| 역할 | 인원 | 단가/월 (시장 평균) | 6개월 합계 |
|---|---|---|---|
| Tech Lead / Senior BE | 1 | 1,000~1,400만 | 6,000~8,400만 |
| Backend Engineer | 1 | 800~1,000만 | 4,800~6,000만 |
| RN Mobile Engineer | 2 | 800~1,000만 | 9,600~12,000만 |
| FE Web Engineer | 1 | 700~900만 | 4,200~5,400만 |
| iOS/Android 컨설턴트 | 0.5 | 1,200만 (외주) | 3,600만 |
| DevOps/SRE | 0.5 | 1,000만 (겸직/외주) | 3,000만 |
| QA Engineer | 1 | 600~800만 | 3,600~4,800만 |
| PM/PO | 1 | 800~1,000만 | 4,800~6,000만 |
| UX/UI 디자이너 | 1 | 600~800만 | 3,600~4,800만 |
| **인력 비용 소계** | **8~9 FTE** | | **약 4.3~5.4억** |

> 단가는 2025~2026 한국 SI/스타트업 중급~시니어 시장 평균 추정. 정규직 vs 외주 mix에 따라 ±30%. 공식 통계 출처는 한정적 — 잡코리아·사람인 연봉 통계 + 시장 통념 기반.
...
**약 4.4~5.7억 (PIA·보험 첫 1년치 별도)**

PIA(자율) 시행 시 +1,500~5,000만, 자동차취급업자 종합보험 첫 1년치 별도 (§5.3 참조).
...
| 역할 | 인원 | 단가/월 |
|---|---|---|
| CS (고객 응대) | 2명 | 600~800만 |
| 정산·제휴 매니저 | 1명 | 500~700만 |
| DevOps 운영 | 0.5명 | 500~700만 |
| **운영 인력 소계** | **3.5명** | **1,500~2,300만/월** |
...
| 시나리오 | 합계 (PG 제외) |
|---|---|
| 저 (저트래픽, OCR/지도 무료 한도) | 약 **2,000만/월** |
| 중 (월 1만 운행 표준) | 약 **2,500만/월** |
| 고 (10만 운행 + 카카오 할인 종료 + 보험 상한) | 약 **3,300만/월** |
...
**손익분기 추정**
- 월 운영비 (인프라+SaaS+인력+보험, PG 제외) ≈ 약 **3,000만/월**
- BEP = 3,000만 ÷ 2.25만 = **약 1,300건/월** (≈ 일 43건)

**규모별 시나리오**
| 운행량 | 월 매출 (수수료) | 월 영업이익 (대략) |
|---|---|---|
| 1,300건 (BEP) | 약 2,925만 | ≈ 0 |
| 1만 건 (MVP 목표) | 약 2.25억 | 약 1.9억 |
...
| **4대보험 사용자 부담분 (~11%)** | 본 인력 단가에는 미포함. 정규직 채용 시 추가 |
