...
| **운영 (매월 반복)** | M5(출시) 이후 | **약 2,000만/월** (저시나리오 950만 ~ 고시나리오 3,800만) | 인프라+SaaS 295만 + 운영 인력 ~1,500만 + 보험 분할 ~200만 |
...
| 4대보험 사용자 부담분 (~10.5%) | | 정규 부분만 적용 | | + 약 2,100~2,600만 |
...
|---|---|---|---|---|
| **저** (저트래픽, OCR/지도 무료 한도, CS 1명) | ~150만 | ~700만 | ~100만 | 약 **950만/월** |
| **중** (월 1만 운행 표준) | ~295만 | ~1,500만 | ~200만 | 약 **2,000만/월** |
| **고** (10만 운행 + 카카오 할인 종료 + 보험 상한 + CS 4명) | ~1,000만 | ~2,500만 | ~300만 | 약 **3,800만/월** |
...
| ~~4대보험 사용자 부담분~~ | v0.2에서 §3.1·§4.2에 **포함** (정규직 부분만, ~10.5%) |
...
| v0.2 | 2026-04-30 | serendibeats | 인건비·운영비 reference 보강·재산정. 한국SW산업협회 공시[27] + 잡코리아·사람인 통계[26] + KT CS 공시[29] 적용. **개발 4.4~5.7억 → 2.7~3.6억** (정규직+외주 분리, 4대보험 사용자 부담 ~10.5% 포함). **운영 2,000~3,300만 → ~2,000만** (CS 단가 시장 평균 적용). BEP 1,300건 → 890건. |
